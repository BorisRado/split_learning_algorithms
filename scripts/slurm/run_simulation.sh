...
for num_clients in 1 2 4 6 8; do

folder_config="hydra.run.dir=outputs/testing/${num_clients}_${server_gpu_type}_${last_client_layer}"
CONFIG="+num_clients=${num_clients} \
    general.num_rounds=5 \
...
srun \
    --ntasks=1 \
    --het-group=0 \
python scripts/py/run_server.py $CONFIG $folder_config &
...
for ((i=0; i<num_clients; i++)); do
    srun \
        ...
        --output=/dev/null \
        --error=/dev/null \
    python scripts/py/run_client.py $CONFIG +client_idx=$i &
done
wait
